// Middleware chain: authenticateUser → resolveOrganization → requireOrganizationMember → requirePermission

const { verifyAccessToken } = require('./tokens');
const { membershipHas } = require('./rbac');

function build(db) {
  // Reads access token from HttpOnly cookie OR Authorization: Bearer header.
  // Attaches req.user (or throws 401 if strict).
  ...
  // Determines the active org from x-organization-id header, ?org query, or user's current_org_id.
  // Loads the membership record so downstream can check role/permissions.
  ...
  return { authenticateUser, resolveOrganization, requireOrganizationMember, requirePermission, requireVerifiedEmail };
}
