...
## Recent Milestones
- **September 2026 — Event Reports**: Admin-only organization-scoped reports tab (`/#reports`), event summary, server-generated two-sheet XLSX and Unicode CSV using existing XLSX. Relationship is `events.id` → `certificates.event_id` with native `_id` compatibility. New events receive verified `organization_id` and `created_by`; legacy unassigned events fail closed and require reviewed ownership backfill. Workers, query indexes, metadata/formula safety, safe binary errors, responsive navigation and download states implemented. 18 focused API/regression tests passed; real browser Excel/CSV downloads and desktop/mobile layout verified. See `EVENT_REPORTS_IMPLEMENTATION.md` for exact files/endpoints/tests and limitations. Follow-ups: ownership backfill (P1), legacy global API organization enforcement (P1), optional queued/scheduled reports (P2).
