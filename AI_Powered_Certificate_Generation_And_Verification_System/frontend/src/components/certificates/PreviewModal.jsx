...
import CertificateSurface from '../../studio/CertificateSurface';

export default function PreviewModal({ cert, template, onClose, apiBase }) {
  if (!cert) return null;
    ...
    <div data-testid="certificate-preview-modal" className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full p-8 relative border-8 border-slate-900 overflow-hidden">
        <button 
          ...
          onClick={onClose}
          className="absolute top-4 right-4 bg-slate-100 text-slate-700 hover:bg-slate-200 p-2 rounded-full font-bold transition z-10"
        >
          ✕
        </button>
