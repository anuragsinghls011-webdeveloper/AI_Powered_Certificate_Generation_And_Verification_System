...
import { 
  ...
  Search, RefreshCw, Layers, Sparkles, Building2, UserCheck, AlertTriangle, Wand2, Rocket, Upload
} from 'lucide-react';
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Try to dynamically detect columns from headers
        ...
        if (hasHeader) {
          ...
          if (firstNameIdx === -1 && lastNameIdx === -1) {
             // Look for generic name, but exclude student_id
        ...
        for (let i = startIndex; i < rows.length; i++) {
          ...
          // Helper to sanitize strings (remove commas to avoid breaking the comma-separated text area)
          const sanitize = (val) => {
