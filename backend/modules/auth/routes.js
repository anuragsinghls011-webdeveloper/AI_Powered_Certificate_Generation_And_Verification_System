...
const { hashPassword, verifyPassword, validatePasswordStrength } = require('./passwords');
const { sendVerificationEmail, sendPasswordResetEmail } = require('./emailService');
const { logAudit } = require('./audit');
const { ROLE_PERMISSIONS, permissionsForMembership } = require('./rbac');
...
function build(db, mw) {
  ...
  (async () => {
    try {
      ...
      await db.collection('sessions').createIndex({ token_hash: 1 });
      await db.collection('sessions').createIndex({ expires_at: 1 }, { expireAfterSeconds: 0 });
      await db.collection('email_verification_tokens').createIndex({ token_hash: 1 });
      ...
      await db.collection('login_attempts').createIndex({ last_attempt: 1 }, { expireAfterSeconds: LOCK_MINUTES * 60 * 4 });
      await db.collection('organizations').createIndex({ slug: 1 }, { unique: true, sparse: true });
      await db.collection('organization_memberships').createIndex({ user_id: 1, organization_id: 1 }, { unique: true });
    } catch (e) { /* indexes may already exist */ }
  })();

  // Ensure a default organization exists — first user becomes super_admin of it.
  async function ensureDefaultOrg(creatorId) {
    let org = await db.collection('organizations').findOne({ is_default: true });
    if (!org) {
      const doc = {
        id: uuidv4(),
  ...
  router.post('/register', registerLimiter, async (req, res) => {
    try {
      ...
      const membership = await db.collection('organization_memberships').findOne({ user_id: userId, organization_id: org.id });
  ...
  router.post('/login', loginLimiter, async (req, res) => {
    try {
      ...
      const organization = user.current_org_id
        ? await db.collection('organizations').findOne({ id: user.current_org_id })
        : null;
  ...
  router.post('/refresh', async (req, res) => {
    try {
      ...
      await db.collection('sessions').updateOne({ id: session.id }, {
  ...
  router.get('/sessions', mw.authenticateUser(), async (req, res) => {
    ...
    const currentSession = currentHash
      ? await db.collection('sessions').findOne({ token_hash: currentHash })
      : null;
  ...
  router.post('/verify-email', async (req, res) => {
    ...
    const rec = await db.collection('email_verification_tokens').findOne({
    ...
    await db.collection('email_verification_tokens').updateOne({ _id: rec._id }, { $set: { used_at: nowIso() } });
  ...
  router.post('/reset-password', async (req, res) => {
    ...
    const rec = await db.collection('password_reset_tokens').findOne({
      token_hash: sha256(token), used_at: null
    });
    if (!rec) return res.status(400).json({ error: 'Invalid or already-used token' });
    if (new Date(rec.expires_at) < new Date()) return res.status(400).json({ error: 'Token expired' });
    ...
    await db.collection('password_reset_tokens').updateOne({ _id: rec._id }, { $set: { used_at: nowIso() } });
  ...
  router.post('/change-password', mw.authenticateUser(), async (req, res) => {
    ...
    await db.collection('users').updateOne({ id: req.user.id }, {
      $set: { password_hash: passwordHash, updated_at: nowIso() }
    });
  ...
  router.patch('/members/:userId/role',
    ...
    async (req, res) => {
      ...
      const result = await db.collection('organization_memberships').updateOne(
